...
echo.
echo  %cYellow%[ 1/3 ]%cReset% Очистка Корзины и Недавних...
rd /s /q "%SYSTEMDRIVE%\$Recycle.Bin" >nul 2>&1
del /f /q "%USERPROFILE%\Recent\*.*" >nul 2>&1
echo  %cGreen%[ OK ]%cReset%

echo  %cYellow%[ 2/3 ]%cReset% Очистка временных файлов Windows...
del /f /s /q "%WINDIR%\Temp\*.*" >nul 2>&1
del /f /s /q "%TEMP%\*.*" >nul 2>&1
echo  %cGreen%[ OK ]%cReset%

echo  %cYellow%[ 3/3 ]%cReset% Очистка кеша обновлений...
